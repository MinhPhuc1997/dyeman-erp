...
export function getDIC(codeTableId) {
  ...
  let resultMap =[];
  // 若字典数据不存在，则等待1.5s后再次执行
  if(codeValueList.length == 0){
    setTimeout(() => {
      getDIC(codeTableId).forEach(item => resultMap.push(item))
    }, 1500);
  }
  ...
  if (store.state.DIC.dicMap[codeTableId]) {
    return store.state.DIC.dicMap[codeTableId];
      ...
      codeValueList.filter(v => v.codetableid.toLocaleLowerCase() === codeTableId).forEach(item => {
      resultMap.push({
        label: item.codename,
        value: item.codeid
      })
  ...
  if(!DB_DICT_ENUM[KEY]){
    ...
    axios.get("/api/" + url).then(res => {
      ...
      axios.get("/api/" + url2).then(Res => {
        ...
        data.forEach(item => {
          DB_DICT_ENUM[KEY].push(item);
        });
      });
    });
  }
  return DB_DICT_ENUM[KEY];
}
  ...
  if(!X_DICT_ENUM[KEY]){
    ...
    axios.get("/api/" + url).then(res => {
      ...
      data.forEach(item => {
        ...
        X_DICT_ENUM[KEY].push(item);
      });
    });
  }

  return X_DICT_ENUM[KEY];
}
