  ...
  >
    <view-container :title="$t('note.title2')" >
      <el-row class="btnList">
        <el-button type="primary" @click="query" v-hasPerm="['quality:weight:list']">{{
          this.$t("public.query")
        }}</el-button>
      </el-row>
        ...
        <el-col :span="12">
          <view-container :title="$t('note.title3')">
            <div class="btnList">
              ...
              <el-button type="primary" @click="openCheck" >{{$t("note.title4")}}</el-button>
            </div>
      ...
      getQcItem().then((res) => {
        ...
        this.qcType.forEach((item) => {
          this.qcForm.group[item - 1] = {
            ...
            column: [
              {
                label: this.$t("clothQc.allCheck") , //"全选",
                prop: item - 1 + "-all",
                type: "switch",
                span: 6,
                dicData: [
