    ...
    <el-row>
      <el-col :span="18">
        ...
        <div class="button-list">
          <el-button type="success" @click="handleQuery" v-hasPerm="['quality:qtyButtonColumn:list']">
            {{ this.$t("public.query") }}
          </el-button>
          <el-popconfirm :title="$t('tipLabel.pirntConfirm')" placement="top-start" @onConfirm="handlePrint"
                         v-hasPerm="['quality:qtyButtonColumn:print']">
    ...
    async handlePrint() {
      ...
      this.$refs.printForm.validate(async (valid, done) => {
        if (valid) {
          ...
          createProProductFlatCardData(Object.assign(SaveParms(this.printForm),{ cardType:1})).then(addRes => {
