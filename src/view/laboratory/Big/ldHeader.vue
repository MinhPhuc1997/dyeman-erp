...
export default {
  ...
  methods: {
    getData() {
      // //获取任务对应的LD通知单数据
      this.$http
        .get("/api/salBigcargo?salBigcargooid=" + this.linkData.salBigcargooid)
        .then((res) => {
          //获取客户信息
          this.$http
          ...
          this.$http
            .get(
              "/api/salBigcargoDtla?salBigcargoDtlaoid=" +
                this.linkData.salBigcargoDtlaoid
            )
            .then((res) => {
              console.log("salBigcargoDtlaoid", res.data);
              this.$http
                .get("/api/salPoDtla?salPoDtlaoid=" + res.data.salPoDtlaFk)
                .then((res) => {
                  console.log("salPoDtlaoid", res.data);
