...
export default {
  ...
  data() {
    ...
    async query() {
      let imp = await axiosGet(API.drawCloth, {vatNo: this.form.vatNo});
      if(imp.data.wfl==null || imp.data.wfl.length==0){
       return this.$tip.warning("Không tim thấy dữ liệu xuất kho cắt mẫu, vui lòng xuất kho trước. 这个缸号没有看到抽布事件， 请先做抽布的事件")
      }
      ...
      this.crud[0].sumWeight = imp.data.netWeightQc;
      this.crud[0].sumWeightLbs = imp.data.netWeightLbsQc;
      this.crud[0].sumLength = imp.data.yardLengthQc;
      this.crud[0].pidCount = imp.data.pidNosQc;

      this.crud[1].sumWeight = imp.data.netWeight;
      this.crud[1].sumWeightLbs = imp.data.netWeightLbs;
      this.crud[1].sumLength = imp.data.yardLength;
      this.crud[1].pidCount = imp.data.pidNos;

      resultCompare(this.crud);
      this.crudError = imp.data.wfl //resultDetail(importTable.data,ledgerTable.data);
      this.crudError.map((e,i)=>{
        e.pidNo=Number(e.productNo.slice(-3).toString())
