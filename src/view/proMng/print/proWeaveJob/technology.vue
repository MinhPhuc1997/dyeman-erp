 ...
 * @LastEditors: Lyl
 * @LastEditTime: 2022-09-28 14:54:41
  ...
  <div class="proWeaveJob-technology" v-loading="loading" :element-loading-text="$t('public.loading')">
    <el-row class="btnList">
      <template v-if="isOutFactory">
        <el-button type="success" @click="handleSave" :disabled="weave.weaveState == 2">{{ $t("public.save") }}</el-button>
...
export default {
  ...
  methods: {
    ...
    async handleSave() {
      ...
      let updateData = Object.assign(this.weave, this.form)
      for (let key in updateData) {
        if (updateData[key] == "undefined") {
          delete updateData[key];
        }
      }
      update(updateData); // 更新主表数据
